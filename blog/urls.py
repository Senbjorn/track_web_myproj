...
urlpatterns = [
    ...
    url(r'^id(?P<pk>\d+)/post/$', login_required(CreatePost.as_view()), name="post_create"),
    url(r'^id(?P<pk>\d+)/edit/$', UpdateBlog.as_view(), name="blog_update"),
    # url(r'^post/list/$', PostList.as_view(), name="post_list"),
    url(r'^id(?P<pk>\d+)/post/comments$', CreatePost.as_view(), name="post_comments"),
